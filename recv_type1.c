...
void ethernet_header(unsigned char* buffer,int buflen)
{
	...
	// xxx
	// if(eth->h_source[0] == SRCMAC0 && eth->h_source[0] == SRCMAC0 && eth->h_source[1] == SRCMAC1 && eth->h_source[2] == SRCMAC2 && eth->h_source[3] == SRCMAC3 && eth->h_source[4] == SRCMAC4 && eth->h_source[5] == SRCMAC5)
	//{
		fprintf(log_txt,"\nEthernet Header\n");
...
void ip_header(unsigned char* buffer,int buflen)
{
	...
	//if(inet_ntoa(source.sin_addr) == "192.168.3.11")
	//{
	fprintf(log_txt , "\nIP Header\n");
	...
	fprintf(log_txt , "\t|-Destination IP    : %s\n",inet_ntoa(dest.sin_addr));
		...
		u_int16_t id16 = ntohs(ip->id);
		char right = id16 & 0x00ff;
		char left = id16 >> 8;
		printf("Key:%d Value:%c \t\n", right, left);
		//printf("IP Header - Identification Field : %d\n",id16 >> 8);
		//printf("IP Header - Identification Field : %d\n",ntohs(ip->id));
	//}
...
int main()
{
	...
	log_txt=fopen("log2.txt","w");
	if(!log_txt)
	{
		printf("unable to open log2.txt\n");
		return -1;
	}

	//msg_txt=fopen("msg2.txt","w");
	//if(!log_txt)
	//{
	//	printf("unable to open msg2.txt\n");
	//	return -1;
	//}
...
void ip_header(unsigned char *buffer, int buflen)
{
    ...
    if(strcmp(inet_ntoa(source.sin_addr), "192.168.3.11") == 0)
    {
        fprintf(log_txt , "\nIP Header\n");
